...
def get_ticket_details(ticket_id: str) -> Dict[str, Any]:
    ...
    if not ticket_id or not str(ticket_id).strip():
        return {"error": "Ticket ID cannot be empty."}
...
def log_ticket_message(ticket_id: str, role: str, content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        session = Session()
        try:
            role_enum = RoleEnum[role.lower()] if role and role.lower() in RoleEnum.__members__ else RoleEnum.ai
            msg = TicketMessage(
...
def get_customer_history(external_user_id: str) -> Dict[str, Any]:
    ...
    from data.models.udahub import User
    try:
        engine = get_udahub_engine()
        Session = sessionmaker(bind=engine)
        session = Session()
        try:
            ...
            if not user:
                return {"user_found": False, "ticket_history": []}
